...
const I18N = {
  en: {
    ...
    userNameGuide: "Tell Katto your name, and Katto will call you for reminders and other moments.",
    ...
    reminderMessagePlaceholder: "What should Katto remind you?",
    reminderAdd: "Add",
    reminderCancel: "Cancel",
    reminderSave: "Save",
    reminderUpdate: "Update",
    reminderClose: "Close",
    reminderEmpty: "Add a reminder and Katto will tell you on time.",
    reminderEdit: "Edit",
  ...
  ko: {
    ...
    userNameGuide: "이름을 알려주면 카토이 알림을 주거나 다양한 상황에서 사용자를 불러줄거예요.",
  ...
  ja: {
    ...
    userNameGuide: "名前を教えると、Katto が通知やいろいろな場面であなたを呼んでくれます。",
...
function applyCatNameSettings(settings) {
  if (!settings) return;
  currentCatName = (settings.name || "Katto").trim() || "Katto";
...
function openCatNameEditor(initialName) {
  ...
  catNameInput.value = (initialName || currentCatName || "Katto").trim();
...
// ── Share recording: current desktop crop + Katto name → vertical video ──
...
async function startShareRecording(durationSec = 5) {
  ...
  const catName = currentCatName || "Katto";
