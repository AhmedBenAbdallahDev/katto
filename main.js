...
process.on("uncaughtException", (error) => {
  logError("[Katto] uncaught exception:", error);
});

process.on("unhandledRejection", (reason) => {
  logError("[Katto] unhandled rejection:", reason);
});
...
try {
  ...
  logWarn("[Katto] uiohook-napi is unavailable:", error && error.message ? error.message : error);
}
...
const I18N = {
  en: {
    ...
    licenseWindowTitle: "Katto License",
    patternEditorTitle: "Katto Pattern Editor",
    mappingEditorTitle: "Katto Cell Mapping Editor",
    appMenuAbout: "About Katto",
    appMenuQuit: "Quit",
    ...
    setCatName: "Set Katto name",
    showCatName: "Show Katto name",
    ...
    accessibilityPermissionMessage: "Allow Katto to react to typing",
    accessibilityPermissionDetail: "Turn on Katto in Accessibility, then restart Katto if typing reactions do not start.",
    inputPermissionTitle: "Input Monitoring may be needed",
    inputPermissionMessage: "Katto still cannot detect typing",
    inputPermissionDetail: "Some macOS environments also require Input Monitoring. Add Katto in Input Monitoring, then restart Katto if typing reactions do not start.",
    ...
    globalInputPermissionMessage: "Katto cannot detect keyboard or wheel input",
    globalInputPermissionDetail: "Restart Katto and check whether Windows security software or system policy is blocking global input hooks.",
    reset: "Reset",
    clear: "Clear",
    delete: "Delete",
    later: "Later",
  ...
  ko: {
    ...
    licenseWindowTitle: "Katto 라이선스",
    patternEditorTitle: "카토 패턴 편집기",
    mappingEditorTitle: "카토 셀 매핑 편집기",
    appMenuAbout: "Katto에 관하여",
    appMenuQuit: "종료",
    ...
    accessibilityPermissionMessage: "카토이 타이핑에 반응하도록 허용해 주세요",
    accessibilityPermissionDetail: "손쉬운 사용에서 Katto을 켜 주세요. 타이핑 반응이 바로 시작되지 않으면 카토을 다시 실행해 주세요.",
    inputPermissionTitle: "입력 모니터링이 필요할 수 있어요",
    inputPermissionMessage: "카토이 아직 키보드 입력을 감지하지 못하고 있어요",
    inputPermissionDetail: "일부 macOS 환경에서는 입력 모니터링도 필요해요. 입력 모니터링에 Katto을 추가한 뒤, 타이핑 반응이 시작되지 않으면 카토을 다시 실행해 주세요.",
    ...
    globalInputPermissionMessage: "Katto이 키보드 또는 휠 입력을 감지하지 못하고 있어요",
    globalInputPermissionDetail: "Katto을 다시 실행해 보세요. 계속 실패하면 Windows 보안 프로그램이나 시스템 정책이 전역 입력 후킹을 막고 있는지 확인해 주세요.",
    reset: "초기화",
    clear: "지우기",
    delete: "삭제",
    later: "나중에",
  ...
  ja: {
    ...
    licenseWindowTitle: "Katto ライセンス",
    patternEditorTitle: "Katto パターンエディター",
    mappingEditorTitle: "Katto セルマッピングエディター",
    appMenuAbout: "Katto について",
    appMenuQuit: "終了",
    ...
    setCatName: "Katto の名前を設定",
    showCatName: "Katto の名前を表示",
    ...
    accessibilityPermissionMessage: "Katto がタイピングに反応できるよう許可してください",
    accessibilityPermissionDetail: "アクセシビリティで Katto をオンにしてください。タイピング反応が始まらない場合は Katto を再起動してください。",
    inputPermissionTitle: "入力監視が必要な場合があります",
    inputPermissionMessage: "Katto がまだキーボード入力を検出できません",
    inputPermissionDetail: "一部の macOS 環境では入力監視も必要です。入力監視に Katto を追加し、タイピング反応が始まらない場合は Katto を再起動してください。",
    ...
    globalInputPermissionMessage: "Katto がキーボードまたはホイール入力を検出できません",
    globalInputPermissionDetail: "Katto を再起動してください。解決しない場合は、Windows のセキュリティソフトまたはシステムポリシーがグローバル入力フックをブロックしていないか確認してください。",
    reset: "リセット",
    clear: "クリア",
    delete: "削除",
    later: "後で",
...
// baseColor: 카토 기본 몸통 색 (타이핑/스트레칭 lerp의 시작점).
...
function customPatternPresetExportName(name) {
  ...
  return `katto-pattern-${safeName}.json`;
}
...
function setCatName(value) {
  const next = String(value || "").trim().slice(0, 24) || "Katto";
  catName = next;
  saveSettings();
...
function attachWindowDiagnostics(win, label) {
  ...
  win.webContents.on("did-fail-load", (_event, errorCode, errorDescription, validatedURL) => {
    logError(`[Katto] ${label} failed to load:`, { errorCode, errorDescription, validatedURL });
  });
  win.webContents.on("render-process-gone", (_event, details) => {
    logError(`[Katto] ${label} render process gone:`, details);
  });
  win.on("unresponsive", () => {
    logWarn(`[Katto] ${label} window became unresponsive`);
  });
}
...
function broadcastMusicState() {
  if (petWin && !petWin.isDestroyed()) {
    petWin.webContents.send("music-state", {
...
function setMusicModeEnabled(enabled) {
  const next = !!enabled;
  if (next === musicModeEnabled) {
    broadcastMusicState();
    return;
  }
  musicModeEnabled = next;
  if (musicModeEnabled) {
    NOW_PLAYING.start((snapshot) => {
      lastMusicSnapshot = snapshot || lastMusicSnapshot;
      broadcastMusicState();
    });
  } else {
    NOW_PLAYING.stop();
    lastMusicSnapshot = { playing: false, title: "", artist: "", source: "" };
  }
  buildAppMenu();
  broadcastMusicState();
}
...
ipcMain.handle("mapping-load", () => {
  try {
    ...
    console.warn("[Katto] failed to load cell mappings:", error && error.message ? error.message : error);
    return null;
  }
});
...
function defaultShareVideoPath(extension) {
  ...
  return path.join(app.getPath("downloads"), `katto-${stamp}.${extension}`);
}
...
// ── Music Mode IPC ──
ipcMain.handle("music-get-state", () => ({ enabled: musicModeEnabled, ...lastMusicSnapshot }));

ipcMain.handle("music-set-enabled", (_evt, enabled) => {
  setMusicModeEnabled(enabled);
  return { enabled: musicModeEnabled, ...lastMusicSnapshot };
});

// Hand the renderer a desktop source id so it can capture system / loopback
// audio for the live equalizer. Reuses the same desktopCapturer plumbing as
// the share-video feature.
ipcMain.handle("music-capture-source", async () => {
  try {
    const sources = await desktopCapturer.getSources({
      types: ["screen"],
      thumbnailSize: { width: 0, height: 0 },
...
ipcMain.on("show-context-menu", () => {
  ...
  const menu = Menu.buildFromTemplate([
    ...
    }] : []),
    {
      ...
      submenu: [
        {
          label: t("musicModeEnable"),
          type: "checkbox",
          checked: musicModeEnabled,
          click: (item) => setMusicModeEnabled(item.checked),
        },
        { type: "separator" },
        {
          label: t("musicTuneIn"),
          enabled: musicModeEnabled,
          click: () => { if (petWin && !petWin.isDestroyed()) petWin.webContents.send("music-tune-in"); },
        },
        {
          label: t("musicStopListen"),
          enabled: musicModeEnabled,
          click: () => { if (petWin && !petWin.isDestroyed()) petWin.webContents.send("music-stop-listen"); },
        },
      ],
    },
    { type: "separator" },
    ...
    { type: "separator" },
    { label: t("madeByCredit"), enabled: false },
    {
      label: t("aboutKatto"),
      click: () => {
        try {
...
function triggerStretchSequence() {
  ...
  // 카토이 현재 있는 디스플레이에서 실행한다. 커서 기준이면 자동 스트레칭이
...
function handleAgentStateEvent(event) {
  ...
  if (state === "thinking" || state === "working") {
    ...
    if (!activeAgentSessions.has(sessionKey)) {
      console.warn(`[Katto] ignored agent complete without active task: ${agentId} ${event.event || ""}`);
      return;
    }
...
function startAgentStateServer() {
  ...
  agentStateServer.on("error", (err) => {
    console.warn("[Katto] agent state server unavailable:", err && err.message);
    agentStateServer = null;
  });
  agentStateServer.listen(AGENT_STATE_PORT, "127.0.0.1", () => {
    console.log(`[Katto] agent state server listening on 127.0.0.1:${AGENT_STATE_PORT}`);
  });
}
...
function installClaudeCodeHooks() {
  try {
    ...
    if (result.added || result.updated) {
      console.log(`[Katto] synced Claude Code hooks (added ${result.added}, updated ${result.updated})`);
    }
  } catch (err) {
    console.warn("[Katto] failed to sync Claude Code hooks:", err && err.message);
  }
}
...
function installAntigravityHooks() {
  try {
    ...
    if (result.added || result.updated) {
      console.log(`[Katto] synced Antigravity hooks (added ${result.added}, updated ${result.updated})`);
    }
  } catch (err) {
    console.warn("[Katto] failed to sync Antigravity hooks:", err && err.message);
  }
}
...
function installCursorHooks() {
  try {
    ...
    if (result.added || result.updated) {
      console.log(`[Katto] synced Cursor hooks (added ${result.added}, updated ${result.updated})`);
    }
  } catch (err) {
    console.warn("[Katto] failed to sync Cursor hooks:", err && err.message);
  }
}
...
function startCodexMonitor() {
  ...
  try {
    ...
    console.warn("[Katto] Codex log monitor unavailable:", err && err.message);
  }
}
...
function startKiroMonitor() {
  ...
  try {
    ...
    console.warn("[Katto] Kiro log monitor unavailable:", err && err.message);
  }
}
...
function startCursorAgentMonitor() {
  ...
  try {
    ...
    console.warn("[Katto] Cursor log monitor unavailable:", err && err.message);
  }
}
...
function registerGlobalShortcuts() {
  ...
  for (const [accelerator, handler] of shortcuts) {
    if (!globalShortcut.register(accelerator, handler)) {
      console.warn(`[Katto] failed to register shortcut: ${accelerator}`);
    }
  }
}
...
function requestAccessibilityPermission() {
  ...
  try {
    ...
    console.warn("[Katto] accessibility permission check failed:", err && err.message);
    return false;
  }
}
...
function startKeyHook() {
  ...
  if (!uIOhook) {
    console.warn("[Katto] global input hook unavailable: uiohook-napi is not loaded");
  ...
  try {
    ...
    console.log("[Katto] global input hook started");
  } catch (err) {
    // Global input hooks may fail when OS permissions or security policy blocks native hooks.
    console.warn("[Katto] global key hook unavailable:", err && err.message);
    if (IS_MAC) {
...
async function checkLicenseValidityNow() {
  // Licensing removed — always treat the app as activated.
  return { ok: true };
}
...
app.whenReady().then(async () => {
  try {
    ...
    if (IS_SMOKE_TEST) {
      logInfo("[Katto] smoke test passed");
      app.quit();
      return;
    }
    app.setName("Katto");
    ...
    // Activation/licensing gate removed — the app boots straight into the cat.
    startLicensedApp();
    ...
    logError("[Katto] startup failed:", error);
    try {
      dialog.showErrorBox("Katto failed to start", error && error.message ? error.message : String(error || "Unknown error"));
    } catch {}
  }
});
